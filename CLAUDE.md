...
This is a **Terraform module** that creates and manages a single `google_compute_network` and multiple `google_compute_subnetwork` resources on GCP. The entire public interface is one input variable (`network_config`) and eight outputs (id, name, project, location, tier, networks, file_shares, create_time). This will be used as a GitHub Repository Template. The actual modules will be implemented separately.
...
cd examples/filestore/basic && terraform init -backend=false && terraform validate

# Run Terratest integration test (requires GCP auth + GOOGLE_CLOUD_PROJECT env var)
cd test && go test -v -timeout 30m -run TestFilestoreInstanceBasic ./filestore_instance_basic_test.go ./helpers_test.go
...
├── main.tf            # Single google_filestore_instance resource
├── variables.tf       # firestore_instance_config object variable with all validations
├── outputs.tf         # Eight instance attribute outputs
├── versions.tf        # Terraform >= 1.3.0, google provider >= 7.23.0
├── examples/
│   └── filestore/basic/  # Reference usage; CI validates this separately
└── test/
    ├── filestore_instance_basic_test.go   # Terratest: creates real instance, asserts outputs, destroys
...
The module uses a single structured `firestore_instance_config` object rather than flat variables. All validation (naming rules, tier enum, project ID format, network configuration values) lives in `variables.tf`.
...
2. **examples-validate** — `init` + `validate` on `examples/filestore/basic` (needs step 1)
